...
export interface ExportProgress {
  ...
  /**
   * Worker 最后**进入**的那一步（`decode:V1` / `encode` / `audio-pull:12` /
   * `finalize` …）。诊断字段，不给用户看。
   *
   * **加它是因为死等**：Safari 上导 30 分钟实测会停在 0% CPU、不抛错、不崩溃、
   * 进度条永远不动（PLAN.md §8 风险 1）。那时唯一的读数是"进度不动了"，而
   * 逐帧循环里有解码、合成、编码、拉音频四个 await，收尾还有一长串完全没有进度
   * 上报的步骤（`finalize` 要写 mp4 索引、`getFile` 要等 OPFS 落盘几百 MB）
   * ——不标出最后进到哪一步，"卡住了"和"卡在哪"就分不开，同上一轮查那 9.3ms
   * 时"只印差值分不清是量歪了还是真被挪了"。
   *
   * 只在事件边界赋一个**常量字符串**，不带分配；收尾那几步各补一次上报，
   * 于是那段静默从"几十秒没有任何消息"变成"每一步都报一次"。
   */
  readonly marker?: string;
  /**
   * 这一条是**心跳**，不是进度：由 Worker 里一个独立定时器发出，不代表任何推进。
   *
   * **没有它，`marker` 是假的分辨率。** 逐帧循环里的进度上报固定发生在
   * `await pumpAudio()` 之后，那一刻 `mark` 还没被下一帧覆盖，于是**每一条正常
   * 进度都带着 `audio`**——它证明管道通，但完全定位不到卡在哪一步。实测踩过：
   * 第一版据此报出"停在 audio"，而那正是稳态下的恒定值。
   *
   * 心跳独立于循环，所以卡住时报的是**此刻**进到哪一步。反过来，心跳也停了
   * 就说明 Worker 的事件循环整个不转了（同步卡死），那本身是另一种诊断。
   *
   * 判"卡住了"的一方**不能把心跳当成推进**，否则永远不会超时——判据要看
   * `encodedFrames` 和 `marker` 有没有变，不是"有没有收到消息"。
   */
  readonly heartbeat?: boolean;
  /**
   * 已经多久没有任何推进（毫秒）。由**主线程**填（`client.ts`），Worker 不产它。
   *
   * **它是提示，不是失败。** 早先版本里我打算让它超时就把导出判死，那会是个
   * 有害的改动：Safari 对**后台标签**的节流会让长片导出正常地停住好几分钟
   * （实测把标签放到前台，同一条 30 分钟片子从"死等"变成 4.4× 跑完），
   * 自动判死等于让"导出期间切去干别的"这个完全正常的用法变成导出失败。
   *
   * 所以只在**页面可见**时才计这个数，并且只用来在界面上说一句"多久没动、
   * 最后停在哪一步"，去留交给用户——同 D19 那条"看得见的降级要标注，不必禁止"。
   */
