...
import type { SourceExtent } from "./transition";
import type { Clip, MediaClip, TextClip, Transition } from "./types";
import { FPS } from "../time/rational";

/**
 * 源片长度 + 两把尺子。缺省是"源片帧率就等于项目帧率"——绝大多数项目的形态，
 * 也是这个文件里除最后那一组之外所有用例的前提。
 */
const ext = (frames: number, over: Partial<SourceExtent> = {}): SourceExtent => ({
  frames,
  fps: FPS.ntsc30,
...
describe("frozenFrames", () => {
  it("余量充足时不定格", () => {
    ...
    const { a, b } = pair();
    const w = transitionWindow(a, b, dissolve(20))!;
    expect(frozenFrames(w, "from", ext(300))).toBe(0);
    expect(frozenFrames(w, "to", ext(300))).toBe(0);
  });
  ...
  it("两段满长素材相邻时两侧都全定格——这是最常见的情形", () => {
    ...
    expect(frozenFrames(w, "from", ext(100))).toBe(10);
    expect(frozenFrames(w, "to", ext(100))).toBe(10);
  });
  ...
  it("余量部分够时只定格差额", () => {
    ...
    const w = transitionWindow(a, b, dissolve(20))!;
    expect(frozenFrames(w, "from", ext(106))).toBe(4); // 需要 10，有 6
    expect(frozenFrames(w, "to", ext(300))).toBe(6); // 需要 10，有 4
  });
  ...
  it("文字片段永远不定格——它的画面是现场生成的", () => {
    ...
    expect(frozenFrames(w, "from", ext(0))).toBe(0);
  });
});
...
describe("availableHandle", () => {
  it("出场看出点之后，入场看入点之前", () => {
    ...
    expect(availableHandle(clip, "from", ext(300))).toBe(170);
    expect(availableHandle(clip, "to", ext(300))).toBe(30);
  });

  it("不返回负数", () => {
    const clip = media({ timelineIn: 0, timelineOut: 100, sourceIn: 0 });
    expect(availableHandle(clip, "from", ext(50))).toBe(0);
  });

  it("**源片帧率 ≠ 项目帧率时余量要换尺子**", () => {
    /*
      25fps 素材放在 30fps 时间轴上：占位 300 帧 = 10 秒 = 250 帧源片。
      源片 375 帧 → 出点之后还剩 125 帧**源片**，换回时间轴帧是 150。

      不换算的旧写法读到的是 375 − (0 + 300) = 75——**少报一半**，界面上那句
      "余量 N 帧"于是是假话，而画面完全正常（定格帧数是显示值，不进渲染）。
    */
    const clip = media({ timelineIn: 0, timelineOut: 300, sourceIn: 0 });
    const mixed = ext(375, { fps: FPS.pal25, timelineFps: FPS.ntsc30 });
    expect(availableHandle(clip, "from", mixed)).toBe(150);
    // 入点侧同理：50 帧源片入点 = 60 个时间轴帧
    const trimmed = media({ timelineIn: 0, timelineOut: 300, sourceIn: 50 });
    expect(availableHandle(trimmed, "to", mixed)).toBe(60);
  });

  it("用满源片的混帧率片段余量为 0，不是负数也不是「还有一截」", () => {
    // 450 帧占位 @30fps = 375 帧源片，正好用满
    const clip = media({ timelineIn: 0, timelineOut: 450, sourceIn: 0 });
    expect(
      availableHandle(clip, "from", ext(375, { fps: FPS.pal25, timelineFps: FPS.ntsc30 })),
    ).toBe(0);
  });
});
...
describe("变速片段的转场余量（D39）", () => {
  ...
  it("定格帧数跟着按速度算", () => {
    ...
    expect(frozenFrames(w, "from", ext(105))).toBe(7);
  });
  ...
  it("0.5× 的片段余量反而更多", () => {
    ...
    expect(availableHandle(clip, "from", ext(101))).toBe(100);
  });
});
