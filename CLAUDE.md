...
后半段：Pixi 后端接进预览和导出（2026-07-27，**D16**；两个前置 `recover()` 与导出复用常驻合成器见 **D15**）→ **一级调色**（亮度 / 对比度 / 饱和度 / 色相，**D17**）→ **3D LUT**（`.cube`，**D18**）。**下一步是 shader 转场**，它要先定转场的时间模型（两个片段重叠，还是零长交界）。界面上还欠的三件小事（自定义字体、时间轴上的关键帧轨、文字阴影颜色）列在 PLAN.md §7 的 M2 小节。

```bash
pnpm dev          # 起开发服务器
pnpm test         # 跑单元测试（352 项：时间基 25 / 状态层 140 / 取样映射 19 / 预设 14 / 合成几何 14 / 色彩矩阵 23 / LUT 22 / 关键帧 31 / 文字断行 23 / 栅格缓存 12 / 常驻量 11 / 混音 6 / 编码延迟 12）
