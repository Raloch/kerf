...
**M2 前半段已完成**：判别联合（D8）→ 图层变换（D9）→ 关键帧求值（D10）→ 接进两条路径 → 文字渲染（D11）。

下一步有两个方向，看优先级：

- **编辑界面**。变换 / 关键帧 / 文字现在 EDL 能存、渲染能读、自检能验，但**没有编辑入口**，只能靠脚本注入。顺带要还 D10 点明的那笔债：**裁入点时要平移关键帧**（`trimClip` 现在没做）。
- **M2 后半段**（滤镜 / LUT / shader 转场）：要先换 Pixi 后端，而换之前**必须在 Safari 上手动跑一遍 M1.6 的 spike**（见 PLAN.md 的 D5 与 §9）。

```bash
pnpm dev          # 起开发服务器
pnpm test         # 跑单元测试（196 项：时间基 25 / 状态层 74 / 取样映射 19 / 预设 14 / 合成几何 14 / 关键帧 27 / 文字断行 23）
