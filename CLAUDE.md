...
下一步是**把关键帧接进 EDL → `ComposeLayer` 的组装**，以及**文字渲染**。这两件事要放在一起做，因为都得改同一处：**导出的逐帧循环必须改成按 `visibleVideoClips` 的图层顺序组装**。现在它是按轨建 reader、reader 出 sample，拿不到"这一层的变换是什么"、也拿不到文字层；各自在导出侧再算一遍就又是两套渲染决策（硬规则 2 的原始形态）。详见 PLAN.md §7 M2。
