...
后半段：Pixi 后端接进预览和导出（2026-07-27，**D16**；两个前置 `recover()` 与导出复用常驻合成器见 **D15**）→ **一级调色**（亮度 / 对比度 / 饱和度 / 色相，**D17**）。**下一步是 LUT**，再之后是 shader 转场（要先定转场的时间模型）。界面上还欠的三件小事（自定义字体、时间轴上的关键帧轨、文字阴影颜色）列在 PLAN.md §7 的 M2 小节。

```bash
pnpm dev          # 起开发服务器
pnpm test         # 跑单元测试（320 项：时间基 25 / 状态层 130 / 取样映射 19 / 预设 14 / 合成几何 14 / 色彩矩阵 23 / 关键帧 31 / 文字断行 23 / 栅格缓存 12 / 常驻量 11 / 混音 6 / 编码延迟 12）
...
- **可动画属性分两组、共用一张关键帧通道表**（`TRANSFORM_PROPERTIES` 六项 → `LayerTransform`，`COLOR_PROPERTIES` 四项 → `ColorAdjust`）。打点 / 删点 / 平移 / 撤销与"作用到摆位还是颜色"无关，分两张表等于全写两遍；但**求值必须分岔**（`resolveTransform` / `resolveColor`）——把 `brightness` 混进 `LayerTransform` 不会报错，合成器不认识那个字段，表现只是"调了色画面没变"。同理 `clearKeyframes` 的烘焙值要按 `isColorProperty()` 决定进 `transform` 还是 `color`。
...
- **Canvas2D 做不了调色，但不能静默丢**。`Compositor.supportsColor` 为 false 时 `composeFrame` 照常画、只忽略 `color`（跑到第三千帧才抛错更坏）；守卫在界面上——`observedCapabilities()` 告诉导出面板本机能力，有调色而做不了就**禁掉导出并写明出路**（同硬规则 10、同 D3 的做法）。**不要**用 `ctx.filter` 在 Canvas2D 上拼一个近似版：那会让同一份 EDL 在两台机器上画出两张画面。
...
| 改了什么 | 必须跑 |
|---|---|
| 导出管道 / 时间基 / 音频 | **M0 自检**（17 项）：生成素材 → 探测 → 导出 trim 区间 → 读回断言。第 17 项比对**素材与成片里第一声的位置差**，抓的是编码延迟没补偿那一类音画偏移 |
| 预览 / 合成 / 文字 / 调色 | **预览 / 导出一致性自检**（16 项）：同一帧两条路径逐像素比对；**带关键帧的变换**在 3 个取样帧上逐帧比对摆位（并断言摆位真的在变——变换被整条丢掉时"两条路径一致"仍然成立）；**文字层**的分区测量（D6）；**一级调色**四条（一致 / 真的生效 / 落在 `colorMatrixOf` 的参照值上 / 没顺手挪动几何） |
| 取帧映射 / EDL / 多轨 | **多片段一致性自检**（23 项）：两片段 + 空档整条导出后比对 7 个取样帧 |
| 合成层 / Pixi 后端 / 调色 | **PixiJS 后端 spike**（21 项）：Worker 里起 WebGL，与 Canvas2D 跑同一份输入比对，外加三类只有 WebGL 才有的失效模式（跨 task 清空 / 上下文丢失**及其恢复** / **上下文预算**——断言的是复用常驻合成器这个生产架构，每轮新建作为对照印在同一行），**图层变换的摆位**（5 个用例 × 两个后端，比对手算位置），以及**一级调色**三条（GPU 像素 == `colorMatrixOf()` 的 CPU 参照实现 / 每个用例真的改变了像素 / 调色之后的恒等帧回到原色） |

第四个原本不是回归自检，而是**换渲染后端之前必须成立的前提**；那些前提已经成立并完成迁移（D15 / D16），它现在是回归护栏。只有 GPU 才能验的东西继续加在这里——调色就是（矩阵语义有单测，但"shader 有没有按那个矩阵算"单测够不着）。
